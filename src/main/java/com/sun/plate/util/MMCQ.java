...
public class MMCQ {
  ...
  public static class VBox {
    ...
    public int[] avg(boolean recompute) {
      if (_avg == null || recompute) {
        int ntot = 0, mult =
            1 << (8 - SIGBITS), rsum = 0, gsum = 0, bsum = 0, hval, i, j, k, histoindex;
        for (i = r1; i <= r2; i++) {
          for (j = g1; j <= g2; j++) {
            for (k = b1; k <= b2; k++) {
  ...
  public static class CMap {
    ...
    public int[] nearest(int[] color) {
      ...
      while (it.hasNext()) {
        DenormalizedVBox vb = (DenormalizedVBox) it.next();
        d2 = Math.sqrt(
            Math.pow(color[0] - vb.getColor()[0], 2) + Math.pow(color[1] - vb.getColor()[1], 2)
  ...
  private static VBox[] medianCutApply(Map<Integer, Integer> histo, VBox vbox) {
    ...
    int rw = vbox.r2 - vbox.r1 + 1, gw = vbox.g2 - vbox.g1 + 1, bw =
        vbox.b2 - vbox.b1 + 1, maxw = Math.max(Math.max(rw, gw), bw);
  ...
  private static Object[] iter(List<VBox> lh, double target, Map<Integer, Integer> histo,
      int nColors, int niters) {
    VBox vbox;
